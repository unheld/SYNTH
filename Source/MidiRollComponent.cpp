...
int MidiRollComponent::pitchToY (int midiNote) const
{
    ...
    const double worldY = (double) kTopMargin + (double) indexFromTop * (double) kNoteHeight;
    return (int) std::round (worldY - scrollY);
}
...
int MidiRollComponent::yToPitch (int y) const
{
    const double worldY = juce::jlimit (0.0, getContentHeight() - 1.0,
                                        (double) y + scrollY);
    const double normalised = (worldY - (double) kTopMargin) / (double) kNoteHeight;
    int row = (int) std::floor (normalised);
...
double MidiRollComponent::xToBeat (int x) const
{
    ...
    const double worldX = (double) x - (double) kLeftMargin;
...
int MidiRollComponent::beatToX (double beat) const
{
    ...
    return (int) std::round (worldX) + kLeftMargin;
}
...
double MidiRollComponent::getPixelsPerBeat() const noexcept
{
    ...
    return (double) availableWidth / getLoopLengthBeats();
}

double MidiRollComponent::getContentHeight() const noexcept
{
    const int rows = (kMaxNote - kMinNote + 1);
    return (double) kTopMargin + (double) rows * (double) kNoteHeight + (double) kTopMargin;
}

void MidiRollComponent::clampVerticalScroll()
{
    const double maxScroll = std::max (0.0, getContentHeight() - (double) getHeight());
...
int MidiRollComponent::hitTestNoteUnlocked (int x, int y) const
{
    for (int i = (int) notes.size() - 1; i >= 0; --i)
    {
        ...
        const int noteWidth  = (int) std::round (n.lengthBeats * getPixelsPerBeat());
...
void MidiRollComponent::paint (juce::Graphics& g)
{
    ...
    // Vertical grid (beats)
    const int maxBeatLines = (int) std::ceil (totalBeats) + 1;
    ...
    for (size_t i = 0; i < noteSnapshot.size(); ++i)
    {
        ...
        const int noteX = beatToX (n.startBeat);
        const int noteW = (int) std::max (8.0, std::round (n.lengthBeats * pixelsPerBeat));
